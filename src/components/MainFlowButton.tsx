...
const MainFlowButton: React.FC = () => {
  ...
  const processEEGData = async () => {
    try {
      ...
      const characterSequence = await eegClassifier.processAllSegments(
        segmentData
      );
      console.log("Classified character sequence:", characterSequence);
      dispatch({ type: "SET_EXTRACTED_TEXT", payload: characterSequence });
      ...
      try {
        ...
        dispatch({
          ...
          payload: llmResponse.correctedText,
        });
      } catch (error) {
        console.log(error);
        dispatch({ type: "SET_PROGRESS", payload: 90 });
        dispatch({ type: "SET_PROCESSED_TEXT", payload: "" });
      }
  ...
  const isAnimated =
    state.isRecording && countdown === 0 && letterCountdown > 0;
