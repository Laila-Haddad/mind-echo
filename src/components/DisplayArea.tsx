...
const DisplayArea: React.FC = () => {
  ...
  if (state.status !== "data_processed" || !state.extractedText) {
    return null;
  }
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      <div className="bg-card border border-border rounded-lg p-8 shadow-lg">
        ...
        {state.processedText && (
          <>
            <h2 className="text-xl font-semibold text-card-foreground mb-4 text-center">
              {t("display_area.aria.corrected_text")}
            </h2>
            <div
              className="text-2xl md:text-3xl lg:text-4xl leading-relaxed text-center text-secondary font-medium min-h-[120px] flex items-center justify-center"
              role="main"
              aria-label={t("display_area.aria.corrected_text")}
            >
              "{state.processedText}"
            </div>
          </>
        ...
        <AccessibleButton
          ...
          aria-label={
            ttsService.isSpeaking()
              ? t("display_area.aria.stop_button")
              : t("display_area.aria.read_button")
          }
        >
          <Speaker className="w-5 h-5" />
          <span className="!m-0">
            {ttsService.isSpeaking()
              ? t("display_area.stop_reading")
              : t("display_area.read_aloud")}
          </span>
        </AccessibleButton>
